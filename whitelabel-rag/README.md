...
|-----------|------------|-------------|
| Backend   | Python FastAPI (uvicorn, port configurable) | REST API server with CORS and modular design |
...
| WebSockets | WebSocket (FastAPI) | Real-time communication between client and server |
...
- `SECRET_KEY` - Secret key for session security (auto-generated by setup scripts)
